...
function Client(config) {
  ...
  // If we passed in a function, then use that to resolve our configuration.
  if (typeof config === 'function') {
    configLoader = config(self);
  // Just use our default client config loader.
  } else {
    configLoader = require('./configLoader')(self, config);
  }

  // Load our configuration.
  process.nextTick(function () {
    configLoader.load(function (err, _config) {
      if (err) {
        self.emit('error', err);
      } else {
 ...
 *
 * @private
 * @param {Object} [config] - Object with client configuration.
 */
Client.prototype._setConfig = function(config) {
  this.config = config;
