...
function CacheHandler(config) {
  ...
  var regionConf = cacheOptions.regions || {};

  _.each(CACHE_REGIONS, function (region) {
    var opts = regionConf[region] || cacheOptions || {};
    opts.store = getCacheCtor( opts && opts.store || cacheOptions.store);
    self.cacheManager.createCache(region, opts || cacheOptions);
  });
}
