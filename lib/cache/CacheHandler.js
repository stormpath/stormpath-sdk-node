...
console.log('****************', href)
  ...
  async.each(
    ...
    function(resource,next) {
      let cache = getCacheByHref(_this.cacheManager, resource.href)
      cache.put(resource.href, resource, _new, next);
    },
    (err) => {
      if (err) {
        return cb(err)
      } else if(utils.isCollectionData(data)) {
        console.log('it is a collection')
        var parentResource = {};
        ...
        let cache = getCacheByHref(_this.cacheManager, data.href)
        cache.put(data.href, parentResource, _new, (err) => {
          if (err) {
console.log('eeeeeeeeeeeeeerr', err)     
            return cb(err)
          }
          cb()
        })
