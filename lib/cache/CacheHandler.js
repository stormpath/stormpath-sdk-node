...
function CacheHandler(config) {
  ...
  self.cacheManager = config.cacheManager || new CacheManager(options);
...
function getCacheByHref(cacheManager, href) {
  ...
  //see: https://github.com/stormpath/stormpath-sdk-node/issues/11
   if (href) {
    ...
    if (cacheManager.options.cacheCustomData && 'customData' === pathArray.slice(-1)[0]) {
      region = pathArray.slice(-3)[0];
    } else {
      region = pathArray.slice(-2)[0];
    }
  }
