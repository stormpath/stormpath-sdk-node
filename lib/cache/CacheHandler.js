...
var CACHE_REGIONS = ['applications', 'directories', 'users', 'groups',
...
CacheHandler.prototype.put = function cacheResource(href, data, _new, cb) {
  ...
  async.each(
    ...
    function(resource,next) {
      getCacheByHref(_this.cacheManager, resource._links.self.href)
        .put(resource._links.self.href, resource, _new, next);
    },
    cb
  );
