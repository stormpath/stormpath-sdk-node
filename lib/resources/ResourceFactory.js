...
var CollectionResource = require('../resource').CollectionResource;
...
function instantiate(InstanceConstructor, data, query, dataStore) {
  ...
  var CollResourceCtor = require('./CollectionResource');

  if (utils.isAssignableFrom(CollResourceCtor, Ctor)) {
  ...
  if (data) {
    if (isCollectionData(data)) {
      resource = new CollResourceCtor(data, q, Ctor, dataStore);
    } else {
