...
DataStore.prototype.exec = function executeRequest(callback){
  ...
  function doRequest(){
    _this.requestExecutor.execute(request, function onGetResourceRequestResult(err, body) {
      if (!err && body) {
        // if(utils.isCollectionData(body) && ctor===ApiKey){
        //   body.items.map(function(item){
        //     item.apiKeyMetaData = {
        //       encryptSecret: query.encryptSecret,
        //       encryptionKeySalt: query.encryptionKeySalt,
        //       encryptionKeySize: query.encryptionKeySize,
        //       encryptionKeyIterations: query.encryptionKeyIterations
        //     };
        //   });
        // }
  ...
  if (Object.keys(query).length > 0 && ctor!==ApiKey){
    return doRequest();
  }
...
DataStore.prototype.createResource = function createResource(/* parentHref, [query], data, [InstanceConstructor], callback */) {
  ...
  _this.requestExecutor.execute(request, function onCreateResourceRequestResult(err, body) {
    ...
    if(returnedResource instanceof ApiKey){
      returnedResource.setApiKeyMetaData(query);
      _this.cacheHandler.put(returnedResource.href, returnedResource, function(err){
        if(err){
          callback(err);
        }else{
          return returnedResource.decrypt(callback);
        }
      });
    }else{
