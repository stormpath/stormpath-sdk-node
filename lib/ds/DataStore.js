 ...
 *
 * @param {Object} config
 * Required configuration object that either provides a `requestExecutor` or values that may be used to instantiate a `RequestExecutor`.
 *
...
/**
 * @param {orderQuery}
 * 'name', 'name asc', {field: -1} === 'field desc'
 */
...
/**
 * @param {string|object} expand
 * 'tenant directory' | {tenant:true} | {tenant: {offset:0, limit:15}}
 *
 * @param {function} cb
 */
 ...
 * The URI of the parent collection under which the new resource will be created as a child. This must always be the first argument.
 *
 * @param {Object} query
 ...
 *
 * @param {InstanceResource} instanceCtor (optional, defaults to ``)
 * The Constructor function to invoke for any instance resource returned by the server. If the request result is a single resource, this function is used to construct the resource. If the request result is a collection, the `instanceCtor` is used to construct each resource in the collection's `items` field.
 *
 * @param {Function} callback
 * The callback function to invoke with the constructed Resource. `callback`'s first argument is an `Error`, the second is the constructed resource. `callback` must always be the last argument.
 *
 * @return {void} no return value; the acquired resource is instead provided to `callback`.
 */
 ...
 * Private method to cache invalidation by HREF.
 *
 * @param {string} href
 * @param {function} callback
 */
