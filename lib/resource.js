...
function Page(collection) {
  ...
  this.each = function eachItem(callback) {
    ...
    if (i === _this.limit) {
      ...
      var nextQuery = util.shallowCopy(_this.query, {});
...
function instantiate(InstanceConstructor, data, query, dataStore) {
  ...
  var Ctor = util.valueOf(InstanceConstructor, InstanceResource);

  if (util.isAssignableFrom(CollectionResource, Ctor)) {
...
function ensureInstanceResources(items, InstanceConstructor, dataStore) {
  ...
  var InstanceCtor = util.valueOf(InstanceConstructor, InstanceResource);
