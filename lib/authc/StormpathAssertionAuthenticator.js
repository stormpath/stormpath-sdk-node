...
StormpathAssertionAuthenticator.prototype.authenticate = function authenticate(stormpath_token, callback) {
  var dataStore = this.dataStore;
  njwt.verify(stormpath_token, this.secret, function (err, jwt) {
    if (err) {
      err.statusCode = 401;
      return callback(err);
    }
    ...
    callback(null, new AssertionAuthenticationResult(
      dataStore, {
        stormpath_token: stormpath_token,
        expandedJwt: jwt,
        account: {
          href: accountHref
        }
