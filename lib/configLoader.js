...
module.exports = function (extendWithConfig) {
  ...
  // Enrich our config with application data from the Stormpath API if we haven't explicitly
  // stated that we want to avoid it (e.g. in tests).
  if (!extendWithConfig.skipRemoteConfig) {
    configLoader.add(new strategy.EnrichClientFromRemoteConfigStrategy(ClientFactory));
  }
