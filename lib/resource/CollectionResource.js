...
function Page(collection) {
   ...
   * @param {function(item, offset, function)} iterator - called for each collection item
  ...
  this.each = function eachItem(page, iterator, callback) {
    ...
    async.each(_this.items, iterator, getNextPage);

    function getNextPage(err) {
      if (err || _this.items.length < _this.limit) {
        return callback(err);
      }
      ...
      nextQuery.offset = _this.offset + _this.items.length;
