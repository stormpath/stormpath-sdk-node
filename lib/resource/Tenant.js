...
Tenant.prototype.verifyAccountEmail = function verifyAccountEmail(token, callback) {
  var self = this;
  var href = "/accounts/emailVerificationTokens/" + (/\//.test(token) ? token.split('/').pop() : token);

  // fix for https://github.com/stormpath/stormpath-sdk-node/issues/28
  var query = Object.create({},{ _ignoreCache: {value: true}});

  return self.dataStore.createResource(href, query, null /* no request body */, require('./Account'), callback);
};
