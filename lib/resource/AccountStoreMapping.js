...
function AccountStoreClassFactory(data, dataStore) {
  ...
  if (/v1\/groups/.test(data.href)) {
    Class = require('./Group');
  }

  if (/v1\/directories/.test(data.href)) {
  ...
  require('./ResourceFactory').instantiate(Class, data, null, dataStore);
}
...
AccountStoreMapping.prototype.getApplication = function getApplication(/* [options,] callback */) {
  ...
  return self.dataStore.getResource(self.application.href, options, require('./Group'), callback);
};
