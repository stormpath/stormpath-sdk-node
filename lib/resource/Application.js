...
Application.prototype.handleIdSiteCallback = function handleIdSiteCallback(/* responseUri, [options,] callback */) {
  var self = this;
  var args = Array.prototype.slice.call(arguments);
  var responseUri = args.shift();
  ...
  self.dataStore.nonceStore.getNonce(nonce,function(err,value){
    if(err){
      ...
      self.dataStore.getResource(accountHref,options,Account,function(err,account){
        if(err){
          cb(err);
        }else{
          cb(null,{
...
Application.prototype.verifyPasswordResetToken = function verifyApplicationPasswordResetToken(/* token, [options,] callback */) {
  var self = this;
  var args = Array.prototype.slice.call(arguments);
  var href = self.passwordResetTokens.href + "/" + args.shift();
  var callback = args.pop();
  var options = (args.length > 0) ? args.shift() : null;
  return self.dataStore.getResource(href, options, callback);
};
