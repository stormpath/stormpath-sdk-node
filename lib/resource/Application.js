...
Application.prototype.getApiKey = function(/* apiKeyId, callback */) {
  ...
  var k = self.href.replace(/applications.*/,'apiKeys/'+apiKeyId);
  return self.dataStore.cacheHandler.get(k,function(err,cachedKey){
    if(cachedKey){
      callback(null, instantiate(require('./ApiKey'), cachedKey, options, self));
    }else{
      self.dataStore.getResource(self.apiKeys.href, options, function(err,result){
        if(err){
          callback(err);
        }else if(result && result.items && result.items.length === 1 ){
          callback(err, instantiate(require('./ApiKey'), result.items[0], options, self));
        }else{
