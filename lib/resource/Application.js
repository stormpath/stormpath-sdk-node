...
Application.prototype.handleIdSiteCallback = function handleIdSiteCallback(responseUri,callback) {
  ...
  if(jwtObjectDecodedValue.body && jwtObjectDecodedValue.body.err){
    return cb(jwtObjectDecodedValue.body.err);
  }
    ...
    if(err){
      ...
      self.dataStore.getResource(accountHref,Account,function(err,account){
        if(err){
          ...
          cb(null,{
            account: account,
            state: decodeURIComponent(jwtObjectDecodedValue.state),
