...
Application.prototype.authenticateApiRequest = function(options,callback) {
  ...
  var parser = new AuthRequestParser(req);
  ...
  if(authHeaderValue){
    if(authHeaderValue.match(/Basic/i)){
      if(grantType){
        authenticator = new OAuthBasicExchangeAuthenticator(application, req);
      }else{
      ...
      authenticator = new OauthAccessTokenAuthenticator(self,authHeaderValue.replace(/Bearer /i,''),callback);
    }
  }else if(accessToken){
  ...
  authenticator.authenticate(function(err,result){
    if(err){
      ...
      if(result.addScope){
        ...
        if(scopeFactory){
          // they want to add custom scope
          result.getAccount(function(err,account){
            if(err){
              callback(err);
            }else{
              var scopes = scopeFactory(account,result.requestedScopes);
              scopes.map(function(scope){
                result.addScope(scope);
              });
              result.tokenResponse = result.getTokenResponse();
              callback(null,result);
            }
          });
        }else{
