 ...
 *
 * For more information about how authorization works, please see
 ...
 * `https://www.example.com/dashboard`. For security reasons, this URL must be
 ...
 * Use this if you already know which Organization the user will be logging into.
 *
 ...
 * For example, `http://example.com/idSiteCallback?jwtResponse=xxx`.
 *
 ...
 * @description
 *
 * Use this function to get an account from a provider directory (Google,
 * Facebook). If you need to fetch a standard account object, please use one of
 * {@link Client#getAccount Client.getAccount(*href*)},
 * {@link Application#getAccounts Application.getAccounts(*query*)},
 * {@link Directory#getAccounts Directory.getAccounts(*query*)}, or
 * {@link Client#getAccounts Client.getAccounts(*query*)}.
 *
 * To use this method, you need to do the following:
 * - Create an application with the provider, and connect it with a Stormpath
 ...
 *
 * Use this method any time you need to register or authenticate a user via
 * a third-party provider. If the user is new, the mirrored acount object will
 * be created in your Stormpath Directory and the result will have a `created`
 * property that indicates this context.  If the user was already mirrored,
 * the account object is simply returned.
 *
 ...
 *
 * @param {Object} [accountData.customData={}]
 * Custom key/value data that you wish to store in this user's custom data
 * resource.
 *
 * @param {String} accountData.email
 * User's email address.
 *
 * @param {String} accountData.givenName
 * User's first name.
 *
 * @param {String} [accountData.middleName]
 * User's middle name.
 *
 ...
 *
 * @param {String} accountData.surname
 * User's last name.
 *
 * @param {String} [accountData.status=UNVERIFIED|ENABLED]
...
/**
 * Retrieves the default Account Store for this Application, which can be a
 ...
 * @param {Function} callback
 * The function to call when the operation is complete.  Will be called
 * with the parameters (err, AccountStore).
 *
 * @example
 *
 * application.getDefaultAccountStore(function(err, accountStore) {
 *   if (!err) {
 *     console.log(accountStore.href);
 *   }
 * });
 */
...
/**
 * Sets the default Account Store for this Application, the Account Store can be
 * a {@link Directory}, {@link Group}, or {@link Organization}.
 *
 * @param {AccountStore|Object} accountStore
 * The Account Store to set as the default Account Store for this application.
 * This can be a materialized {@link Directory}, {@link Group},
 * {@link Organization}, or object literal with an `href` property that
 * identifies the account store.
 *
 * @example
 *
 ...
 * Directory} resource (not the account store mapping).  This will be fixed in
 * version 1.0.0.
 *
 * @param {GetResourceOptions} [getResourceOptions]
 * Query options for the request, e.g. to expand the directory.
 *
 * @param {Function} callback
 * The function to call when the operation is complete.  Will be called
...
/**
 * Sets the default group store for this Application, the default group store
 * must be a {@link Directory}.  This is the {@link Directory} that new groups
 * will be created in when using {@link Application#createGroup
 * Application.createGroup()}.
 *
 ...
 *
 * client.getDirectory(directoryHref, function(err, directory}{
 *   application.setDefaultGroupStore(directory, function (err) {
 *     if (!err) {
