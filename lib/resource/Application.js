...
Application.prototype.authenticateOauthTokenRequest = function(request, callback) {
  var self = this;
  var parser = new AuthRequestParser(request);
  var authHeaderValue = parser.getHeaderValue('authorization');
  var grantType = parser.getParam('grant_type');
...
Application.prototype.authenticateOauthRequest = function(request,callback) {
  var self = this;
  var parser = new AuthRequestParser(request);
  var authHeaderValue = parser.getHeaderValue('authorization');
  var accessToken = parser.getParam('access_token');

  if(authHeaderValue.match(/Bearer/i)){
    return new OauthAccessTokenAuthenticator(self,authHeaderValue.replace(/Bearer /i,''),callback);
  }else if(accessToken){
    new OauthAccessTokenAuthenticator(self, accessToken, callback);
  }else{
    return callback(new ApiAuthRequestError('Must provide access_token'));
  }
};

Application.prototype.authenticateBasicRequest = function(request,callback) {
  var self = this;
  var parser = new AuthRequestParser(request);
  var authHeaderValue = parser.getHeaderValue('authorization');
