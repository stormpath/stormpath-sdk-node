...
Application.prototype.handleIdSiteCallback = function handleIdSiteCallback(responseUri,callback) {
  ...
  var clientState = jwtObjectDecodedValue.state;

  self.dataStore.nonceStore.getNonce(nonce,function(err,value){
    if(err){
      cb(err);
    }else if(value){
      cb(new Error("JWT has already been used."));
    }else{
      self.dataStore.nonceStore.putNonce(nonce,utils.noop);

      if(jwtObjectDecodedValue.aud !== apiKeyId){
        return cb(new Error("The client used to sign the jwrResponse is different than the one used in this datasore."));
      }

      if(!utils.isNumber(jwtObjectDecodedValue.exp) || (utils.nowEpochSeconds() > jwtObjectDecodedValue.exp)){
        return cb(new Error("JWT has expired"));
      }

      var data = {};
      data.account = { href: accountHref };
      data.idSiteResponse = jwtObjectDecodedValue;
      data.clientState = clientState;

      cb(null, new AuthenticationResult(data));
    }
  });
};
