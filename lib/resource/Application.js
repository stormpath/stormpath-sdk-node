...
Application.prototype.createIdSiteUrl = function createIdSiteUrl(_options) {
  ...
  if(!options.cb_uri){
    throw new Error('cb_uri URI must be provided and must be in your ID Site whitelist');
  }
...
Application.prototype.handleIdSiteCallback = function handleIdSiteCallback(/* uri, [responseMode], callback */) {
  var self = this;
  ...
  var nonce, accountHref, clientState;

  var jwtObjectDecodedValue = self._decodeJwt(token,secret);

  if(jwtObjectDecodedValue instanceof Error){
    return callback(jwtObjectDecodedValue);
  }

  nonce = jwtObjectDecodedValue.irt;
  accountHref = jwtObjectDecodedValue.sub;
  clientState = jwtObjectDecodedValue.state;

  self.dataStore.nonceStore.getNonce(nonce,function(err,value){
    if(err){
      callback(err);
    }else if(value){
      callback(new Error("JWT has already been used."));
    }else{
      self.dataStore.nonceStore.putNonce(nonce,utils.noop);

      if(jwtObjectDecodedValue.aud !== apiKeyId){
        return callback(new Error("The client used to sign the jwrResponse is different than the one used in this datasore."));
      }

      if(!utils.isNumber(jwtObjectDecodedValue.exp) || (utils.nowEpochSeconds() > jwtObjectDecodedValue.exp)){
        return callback(new Error("JWT has expired"));
      }
      ...
      if(responseMode==='account'){
        ...
        return callback(null,jwtObjectDecodedValue);
      }else{
