...
Application.prototype.getAccounts = function getApplicationAccounts(/* [options,] callback */) {
  var args = utils.resolveArgs(arguments, ['options', 'callback'], true);
  return this.dataStore.getResource(this._links.users.href, args.options, require('./Account'), args.callback);
};
...
Application.prototype.getApiKey = function getApiKey(apiKeyId, options, callback) {
  ...
  var userQuery = '/users?search=profile.stormpathApiKey_1%20sw%20%22' + apiKeyId + '%22';

  return dataStore.getResource(userQuery, require('./Account'), function (err, accounts) {
    if (err) {
      return cb(err);
    }
    ...
    var matchedKeyParts = Object.keys(account.profile).reduce(function (match, key) {
