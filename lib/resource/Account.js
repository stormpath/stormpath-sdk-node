...
Account.prototype.createApiKey = function createApiKey(/* [options] , callback */) {
  ...
  return self.dataStore.createResource(self.apiKeys.href, options, require('./ApiKey'), callback);
};

Account.prototype.getApiKeys = function getApiKeys(/* callback */) {
  var self = this;
  var args = Array.prototype.slice.call(arguments);
  var callback = args.pop();
  var options = (args.length > 0) ? args.shift() : null;

  return self.dataStore.getResource(self.apiKeys.href, options, require('./ApiKey'), callback);
};
