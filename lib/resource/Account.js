...
/**
* Create a {@link Factor} for this account, which can be used in multi-factor
* authentication.
*
* @param {FactorData} factor
* The data for the new factor object. It must have a type property, which must
* be either "SMS" or "google-authenticator" (capitalization irrelevant). The
* former will produce an {@link SmsFactor}, while the latter will produce a
* {@link GoogleAuthenticatorFactor}. Can optionally contain a challenge, which
* is then automatically created.
*
* @param {Object} [factor.challenge]
* A {@link Challenge} object to be created for this factor.
*
* @param {String} [factor.challenge.message]
* The message for the challenge. See {@link Challenge} for details.
*
* @param {Function} callback
* The function to call when the operation is complete. Will be called with the
...
* };
*
* account.createFactor(factorData, function(err, createdFactor) {
*   console.log(createdFactor);
* });
*/
Account.prototype.createFactor = function createFactor(/* factor, callback */) {
  var args = utils.resolveArgs(arguments, ['factor', 'callback']);
  var FactorCtor = getFactorConstructor.call(this, args.factor);

  return this.dataStore.createResource(this.factors.href, {}, args.factor, FactorCtor, args.callback);
};
...
*
* @param {CollectionQueryOptions} collectionQueryOptions
* Options for expanding the query. Other operations are not supported.
*
* @param {Function} callback
