...
/**
 * If the Group has been constructed with an Okta group, call this function to
 * convert the object into one that looks like a Stormpath group.
 */
Group.prototype.convertFromOktaGroup = function convertFromOktaGroup() {
  this.name = this.profile.name.replace(/group:[^:]+:/, '');
  this.description = this.profile.description;
};

/**
 * Takes the callback result from a group collection getter, e.g. account.getGroups(),
 * and removes the groups that dont start with "group:".  That prefix is how we know that
 * the group came from a stormpath import.
 *
 * Use this like a static method of Group.
 */
Group.prototype.oktaGroupCollectionFilter = function oktaGroupCollectionFilter(callback, err, groups) {
  if (err) {
    return callback(err);
  }
  groups.items = groups.items.filter(function (group) {
    return group.profile.name.match(/^group:/);
  }).map(function (group) {
    group.convertFromOktaGroup();
    return group;
  });
