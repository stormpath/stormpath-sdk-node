...
function cachePut(href, data, _new, cb) {
    ...
    if (val && val.href) {
      ...
      if (val.items) {
        ...
        _.each(val.items, function (item) {
          cachePut.call(_this, item.href, item, utils.noop);
          v2.items.push({href: item.href});
        });
      } else {
        if (Object.keys(val).length > 1) {
          cachePut.call(_this, val.href, val, utils.noop);
        }
      }
    } else {
      v2 = val;
  ...
  getCache.call(_this, href).put(href, resourceData, _new, cb);
}
...
DataStore.prototype.getResource = function getResource() {
  ...
  function processCallback(err, entry, cb){
    if (entry){
      var resourceFromCache = instantiate(InstanceCtor, entry, query, _this);
      return cb(null, resourceFromCache);
    }
    return cb(err);
  }

  // todo: tbd - ignore cache if query provided
  cacheGet.call(_this,href, function tryGetEntryFromCache(err, entry){
    if (err || entry){
      return processCallback(err, entry, callback);
    }
    _this.requestExecutor
      .execute(request, function onGetResourceRequestResult(err, body) {
        if (!err && body) {
          cachePut.call(_this, href, body, utils.noop);
        }
        return processCallback(err, body, callback);
      });
  });
};
...
DataStore.prototype.deleteResource = function deleteResource(resource, callback) {
  ...
  this.requestExecutor.execute(request, function onDeleteResourceRequestResult(err) {
    ...
    return callback(null); //no body for the callback
  });
};
