  ...
  $.fn.toc = function(options) {
    ...
    return this.each(function() {
      ...
      headings.each(function(i, heading) {
        ...
        var headerText = opts.headerText(i, heading, $h);

        // Find headings that are a method signature
        var methodSignatureRegex = /\([^\)]+\)/;

        var signatureMatch = headerText.match(methodSignatureRegex);

        var span = $('<span/>');

        if(signatureMatch) {
          var signature = signatureMatch[0];
          var methodName = headerText.replace(signature, '');
          span.append($('<span/>').text(methodName + ' '));
        } else {
