...
A `Group` is a collection of [Account](account)s within a [Directory](directory) that are often used for authorization and access control in an [Application](application). In Stormpath, the term Group is synonymous with 'Role'.

For example, an application will often check which groups are assigned to an account, and perform access control decisions based on which groups the account has or not.
